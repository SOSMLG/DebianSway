...
# an Xorg conf snippet. Only applied if you say yes — on
# sway/Wayland this file is ignored, so prefer the input
# block already baked into ~/.config/sway/config instead.
# =======================================================
...
EOF
    ...
    if [ -f "$XORG_CONF_FILE" ]; then
        ...
        log_warn "Note: this file only applies under Xorg. On sway/Wayland, libinput"
        log_warn "options go in the input block of ~/.config/sway/config instead — already"
        log_warn "covered by this toolkit's config (tap-to-click on, natural scroll off)."
    else
