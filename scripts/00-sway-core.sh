...
#
# TLP / battery thresholds live in usefulApps.sh, not here.
# =======================================================
set -uo pipefail
...
# ---------------------------------------------------------------------------
# 7. Bake configs into ~/.config (catppuccinThemed, from catppuccinSway.sh)
# ---------------------------------------------------------------------------
...
if [ -d "$CONFIGS_SRC" ]; then
    ...
    log_info "Run scripts/26-debswayCli.sh to install the debsway theme CLI."
else
    log_warn "configs/ not found next to scripts/ — skipping (run catppuccinSway.sh instead)."
fi
...
#    set in hardwareSupport.sh covers them all. Run it next.
# ---------------------------------------------------------------------------
log_ok "Sway core installed."
log_warn "Next: scripts/01-backports.sh (repos) + scripts/hardwareSupport.sh (firmware)."
