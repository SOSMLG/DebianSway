...
# CLI from scripts/26-debswayCli.sh, running OpenCode in an alacritty terminal)
...
# ---------------------------------------------------------------------------
# Hotkey: Super+A -> open OpenCode via the debsway CLI (`debsway agent`)
# whenever the debsway CLI is installed (scripts/26-debswayCli.sh);
# otherwise fall back to a bare terminal launch.
#
# On sway (Wayland), global hotkeys are just bindsym lines in
# ~/.config/sway/config. We append one (idempotently) rather than writing a
# .desktop/global-shortcut registry like KDE's kglobalaccel would.
# ---------------------------------------------------------------------------
SWAY_CONFIG="$HOME/.config/sway/config"
if [ -f "$SWAY_CONFIG" ] && ask "Bind Super+A to launch OpenCode?"; then
    if grep -qE "debsway agent|opencode" "$SWAY_CONFIG" 2>/dev/null; then
        log_ok "Super+A binding already present in $SWAY_CONFIG."
    else
        cat >> "$SWAY_CONFIG" << 'EOF'

# OpenCode (Super+A) — added by scripts/aiOpencode.sh
...
if ask "Install a system 'skill' file so AI tools know this is a Debian/Sway box?"; then
    SKILL_SRC="$SCRIPT_DIR/skills/deb-sway-thinkpad-SKILL.md"
    if [ -f "$SKILL_SRC" ]; then
