...
# -------------------------------------------------------
# Legacy alias of the debsway theme engine (scripts/26-debswayCli.sh installs
...
# the Sway/Waybar/Wofi/Foot/Mako/SwayOSD configs in configs/, installs the
...
if [ -d "$CONFIGS_SRC" ]; then
    ...
    for d in sway waybar wofi foot mako swayosd wlogout kanshi gammastep; do
...
if [ -f "$SWAY_CONFIG" ] && [ -d "$CURSOR_TARGET" ]; then
    if grep -q "xcursor_theme Catppuccin" "$SWAY_CONFIG" 2>/dev/null; then
        ...
        printf '\n# Catppuccin cursor theme (set by catppuccinSway.sh)\nseat * xcursor_theme Catppuccin-Mocha-Red 24\n' >> "$SWAY_CONFIG"
