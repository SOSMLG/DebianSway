...
# -------------------------------------------------------
# Installs:
#   foot        (default terminal, replaces alacritty as $term)
...
install_pkgs "Shell upgrade" \
    foot swayosd cliphist swappy wlogout playerctl pamixer \
...
if [ -d "$CONFIGS_SRC" ]; then
    ...
    for d in sway waybar wofi foot mako swayosd wlogout kanshi gammastep environment.d; do
...
log_warn "Next: scripts/26-debswayCli.sh installs the debsway CLI + theme engine."
