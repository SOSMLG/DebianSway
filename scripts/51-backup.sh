...
do_backup() {
    ...
    local archive="$BACKUP_ROOT/deb-sway-config-backup-${stamp}.tar.gz"
    ...
    [ -f "$HOME/.local/state/deb-sway-thinkpad/last-run.log" ] \
        && cp "$HOME/.local/state/deb-sway-thinkpad/last-run.log" "$archive.log"
