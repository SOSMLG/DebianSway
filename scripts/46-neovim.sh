...
#      ~/.config/nvim/lua/plugins/debsway-pinned.lua. On Neovim
...
PIN_FILE="$NVIM_DIR/lua/plugins/debsway-pinned.lua"
...
if [ "$PIN_MODE" -eq 1 ]; then
    if [ -f "$PIN_FILE" ] && grep -q "deb-sway-thinkpad 46-neovim" "$PIN_FILE" 2>/dev/null; then
        log_ok "v14 pin already present."
    else
        mkdir -p "$NVIM_DIR/lua/plugins"
        cat > "$PIN_FILE" << 'EOF'
-- Written by deb-sway-thinkpad 46-neovim.sh — Debian ships Neovim 0.10
